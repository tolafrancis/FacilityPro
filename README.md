...
> **Universal IoT layer (0077).** Devices from any manufacturer (Schneider, Siemens, Daviteq, EPCB, Milesight, Dragino…) are described by catalog **models** and **data points**, and every reading is normalised to standard metrics and units on arrival (°F→°C, kW→W, Wh→kWh…). Devices sit on the building → floor → zone → room tree, show live data (Realtime), history charts, severity alerts (acknowledge, auto-resolve, escalation) and **commands** (setpoints, on/off; role-, range- and staleness-checked, expiring, audited). Technicians can now open Devices; keys and configuration stay admin/manager-only. Modbus TCP/RTU and on-site MQTT run through the new edge gateway ([`iot-gateway/`](iot-gateway/README.md)); LoRaWAN arrives from The Things Stack or ChirpStack webhooks. Deploy `iot-ingest` (`--no-verify-jwt`) and `iot-api`. Architecture, protocol status and roadmap: [`docs/IOT_ARCHITECTURE.md`](docs/IOT_ARCHITECTURE.md).
