...
async function fetchDocuments(orgId: string | undefined) {
  if (!orgId) return [] as DocumentItem[];
...
export default function Documents() {
  ...
  const { data: documents = [] } = useQuery({
    queryKey: ['documents', currentOrg?.id],
    queryFn: () => fetchDocuments(currentOrg?.id),
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('fp_documents').insert({
